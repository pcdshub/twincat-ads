                                     ...
                                     0) /* Default stack size*/
{
  ...
  //* Create the thread that does callback triggers */
  ...
  if (status)
  {
    throw std::runtime_error(string_format("%s:%s: epicsThreadCreate failure for TriggerEpicsIoIntrCallbacksThread.\n", driverName, __func__));
  }
  ...
  for (;;) {
	  ...
	  asynPrint(pasynUserSelf, ASYN_TRACE_ERROR, 
        ...
        {
          timeval tableLoadStart, tableLoadEnd;
          gettimeofday(&tableLoadStart, NULL);
          if (adsSymbolParserMap_.at((uint16_t)amsport).load(adsClientPort, {remoteNetId_, (uint16_t)amsport}))
          {
            asynPrint(pasynUserSelf, ASYN_TRACE_ERROR, 
                "%s:%s: failed to load ads symbol map for %u.\n", 
                driverName, functionName, amsport);
          }
          gettimeofday(&tableLoadEnd, NULL);
          printf("Table load time in microseconds: %ld microseconds\n",
                    ((tableLoadEnd.tv_sec - tableLoadStart.tv_sec)*1000000L
                  +tableLoadEnd.tv_usec) - tableLoadStart.tv_usec
                  );
        }
	  return;
      }
...
adsAsynPortDriver::~adsAsynPortDriver()
{
  ...
  for(int i=0;i<adsParamArrayCount_;i++){
    adsDelDataCallback(amsClientPort, &adsParamArray_[i],true);  //Block error messages
    adsReleaseSymbolicHandle(amsClientPort, &adsParamArray_[i],true);    //Block error messages
...
void adsAsynPortDriver::report(FILE *fp, int details)
{
  ...
  if(details>=2){
    ...
    for(int i=0; i<adsParamArrayCount_;i++){
      adsParamInfo *paramInfo=&adsParamArray_[i];
...
asynStatus adsAsynPortDriver::drvUserCreate(asynUser *pasynUser,const char *drvInfo,const char **pptypeName,size_t *psize)
{
  ...
  adsParamInfo *paramInfo=&adsParamArray_[adsParamArrayCount_];
...
asynStatus adsAsynPortDriver::updateParamInfoWithPLCInfo(uint16_t amsClientPort, adsParamInfo *paramInfo)
{
  ...
  // printf("Elapsed time in microseconds: %ld microseconds\n",
...
asynStatus adsAsynPortDriver::adsAddToBulkRead(uint16_t amsClientPort, adsParamInfo* paramInfo)
{
  std::lock_guard<std::mutex> lg(bulkReadInfoMutex_);
    if (paramInfo->bulkIndex < 0) { /* Not assigned yet, find one! */
        int i = 0;
        auto it = amsPortToNotFullBulkReadInfoMap_.find(paramInfo->amsPort);
        if (it != amsPortToNotFullBulkReadInfoMap_.end() && bulk[it->second].cnt < BULKSIZ)
        {
          i = amsPortToNotFullBulkReadInfoMap_[bulk[i].amsPort];
          if (i >= BULKSIZ)
          {
            for (++i; i < MAXBULK; i++) {
                ...
                if (bulk[i].cnt == 0 || (bulk[i].cnt != BULKSIZ &&
                    ...
                    {
                      amsPortToNotFullBulkReadInfoMap_[bulk[i].amsPort] = i;
                      break;
                    }
            }
          }
    ...
    // Update the information for an allocated element.
...
asynStatus adsAsynPortDriver::getRecordInfoFromDrvInfo(const char *drvInfo,adsParamInfo *paramInfo)
{
  ...
  std::string varName = paramInfo->plcAdrStr;
  auto adsSymbolParserMapIt = adsSymbolParserMap_.find(paramInfo->amsPort);
  if (adsSymbolParserMapIt != adsSymbolParserMap_.end())
  {
    auto adsSymbolEntryExpanded = adsSymbolParserMapIt->second.lookup(varName);
    if (adsSymbolEntryExpanded)
    {
      switch((AdsDatatypeId)adsSymbolEntryExpanded->dataType) {
      case AdsDatatypeId::Void:
          paramInfo->asynType = asynParamType::asynParamNotDefined;
          break;
      
      case AdsDatatypeId::Int8:
      case AdsDatatypeId::UInt8:
          if (adsSymbolEntryExpanded->isArray)
            paramInfo->asynType = asynParamType::asynParamInt8Array;
          else
            paramInfo->asynType = asynParamType::asynParamInt32;
          break;
      
      case AdsDatatypeId::Int16:
      case AdsDatatypeId::UInt16:
          if (adsSymbolEntryExpanded->isArray)
            paramInfo->asynType = asynParamType::asynParamInt16Array;
          else
            paramInfo->asynType = asynParamType::asynParamInt32;
          break;
      ...
      case AdsDatatypeId::UInt32:
          if (adsSymbolEntryExpanded->isArray)
            paramInfo->asynType = asynParamType::asynParamInt32Array;
          else
            paramInfo->asynType = asynParamType::asynParamInt32;
          break;
      ...
      case AdsDatatypeId::UInt64:
          if (adsSymbolEntryExpanded->isArray)
            paramInfo->asynType = asynParamType::asynParamInt64Array;
          else
            paramInfo->asynType = asynParamType::asynParamInt64;
          break;
      
      case AdsDatatypeId::Real32:
          if (adsSymbolEntryExpanded->isArray)
            paramInfo->asynType = asynParamType::asynParamFloat32Array;
          else
            paramInfo->asynType = asynParamType::asynParamFloat64;
          break;

      case AdsDatatypeId::Real64:
          if (adsSymbolEntryExpanded->isArray)
            paramInfo->asynType = asynParamType::asynParamFloat64Array;
          else
            paramInfo->asynType = asynParamType::asynParamFloat64;
          break;
 ...
 * is reserved for this interface (and also &adsParamArray_[0]).
 * \param[in] pasynUser Pointer to asyn user structure
...
asynStatus adsAsynPortDriver::writeInt32(asynUser *pasynUser, epicsInt32 value)
{
  ...
  paramInfo=&adsParamArray_[paramIndex];
...
asynStatus adsAsynPortDriver::adsGenericArrayRead(asynUser *pasynUser,long allowedType,void *epicsDataBuffer,size_t nEpicsBufferBytes,size_t *nBytesRead)
{
  ...
  if(paramIndex>=paramTableSize_){
    asynPrint(pasynUser, ASYN_TRACE_ERROR, "%s:%s: adsParamArray index (pasynUser->reason) out of range\n", driverName, functionName);
    pasynUser->alarmStatus=READ_ALARM;
    pasynUser->alarmSeverity=INVALID_ALARM;
    return asynError;
  }

  adsParamInfo *paramInfo=&adsParamArray_[paramIndex];
...
asynStatus adsAsynPortDriver::readInt8Array(asynUser *pasynUser,epicsInt8 *value,size_t nElements,size_t *nIn)
{
  ...
  if(pasynUser->reason >= adsParamArrayCount_){
    asynPrint(pasynUser, ASYN_TRACE_ERROR, "%s:%s: adsParamArray index out of range\n", driverName, functionName);
    return asynError;
  }
  ...
  if(adsParamArray_[pasynUser->reason].plcDataType==ADST_STRING){
    allowedType=ADST_STRING;
  }
  else if(adsParamArray_[pasynUser->reason].plcDataType==ADST_BIT){
...
asynStatus adsAsynPortDriver::adsGetSymInfoByName(uint16_t amsClientPort, uint16_t amsPort,const char *varName, adsSymbolEntry *info,long *errorCode)
{
  ...
  if (adsSymbolParserMapIt != adsSymbolParserMap_.end())
  {
    ...
    auto adsSymbolEntryExpanded = adsSymbolParserMapIt->second.lookup(variableName);
    if (adsSymbolEntryExpanded)
    {
...
asynStatus adsAsynPortDriver::adsUpdateParameter(adsParamInfo* paramInfo,const void *data,size_t dataSize)
{
  ...
  paramInfo->callbackRequested = true;
    ...
    case ADST_INT8:
      switch(paramInfo->asynType){
        case asynParamInt8Array:
          ...
          break;
        default:
          asynPrint(pasynUserSelf, ASYN_TRACE_ERROR, "%s:%s: Type combination not supported. PLC type = %s, ASYN type= %s, Name= %s\n", driverName, functionName,adsTypeToString(paramInfo->plcDataType),asynTypeToString(paramInfo->asynType), paramInfo->plcAdrStr);
          return asynError;
          break;
      }
      break;
...
{
  const char* functionName = "adsAddRoute";
  asynPrint(pasynUserSelf,ASYN_TRACE_FLOW, "%s:%s:\n", driverName, functionName);
